...
一款轻量、专注的 Windows 桌面待办任务工具。支持多行批量输入、双击编辑、任务置顶、工作/生活/购物标签分类（长按拖排）、勾选删除线动画、本地持久化、一周自动过期、窗口一键置顶，以及基于 GitHub Releases 的自动更新。
...
| 🏷️ **标签分类** | 内置「工作 / 生活 / 购物」三标签；列表按标签分组，无标签任务在「未分类」最后展示 |
...
| 💾 **本地持久化** | 任务与标签顺序写入 `localStorage`，关闭软件不丢失 |
| ⏰ **一周自动过期** | 创建起 7 天后自动删除，卡片显示「还剩 N 天 / 即将过期」 |
...
| **Storage API** | `localStorage` 持久化任务与标签顺序 |
...
| | `setTaskTag` / `render` | 打标签；按 `tagOrder` 分组，未分类置底 |
...
["work", "life", "shopping"]   // 决定分组展示顺序；未分类始终在最后
```
...
**过期逻辑**：`Date.now() - createdAt >= 7 天` 时删除；启动时清理，之后每小时巡检。

### 标签与列表渲染流程

```
新建任务：
  标签栏选中 draftTag（可空）→ Enter 添加 → task.tag = draftTag → render()

改标签：
  点击卡片标签按钮 → 下拉选择 → setTaskTag → render()
...
const EXPIRE_DAYS   = 7;
const LONG_PRESS_MS = 420;   // 标签长按触发拖拽的阈值
```
